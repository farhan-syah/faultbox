...
/// If several layers each emit for the same underlying failure, `faultbox` files
/// a separate report for each — it cannot tell that they share a root cause.
///
/// This is the failure mode to design against: report at the detection site and
/// propagate elsewhere. Pinned here so the behaviour is a documented
/// consequence rather than a surprise in production.
#[test]
